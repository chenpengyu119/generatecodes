...
import com.travelsky.basedata.common.constant.DeleteFlag;
...
public class ${class_name}ServiceImpl implements ${class_name}Service{
    ...
    @Autowired
    //声明属性
    private ${class_name}DataService ${object_name}DataService;
    ...
    public List<${class_name}Dto> getAll${class_name}s(){
        ...
        //查询所有未删除得数据
        List<${class_name}> list = ${object_name}Repository.findAllByDelflag(DeleteFlag.UN_DELETED);
        //如果查询到数据，进行数据转换并返回
    ...
    public ${class_name}Dto get${class_name}ById(Long id){
        //根据id主键从数据库查询所有未删除的数据
        Optional<${class_name}Dto> ${object_name}Dto = ${object_name}DataService.findByIdAndDelflag(id, DeleteFlag.UN_DELETED);
        if(!${object_name}Dto.isPresent()){
            //抛出资源没有找到的异常
            throw new ResourceNotFoundException(ExceptionMessageFormat.idNotFoundMessage(id));
        }
        //获取实体
        return ${object_name}Dto.get();
    }
    ...
    public void add${class_name}(${class_name}Dto ${object_name}Dto){
        ...
        //设置删除标志位为未删除状态
        ${object_name}.setDelflag(DeleteFlag.UN_DELETED);
        //数据保存
    ...
    public void update${class_name}(${class_name}Dto ${object_name}Dto){
        //根据唯一编码从数据库查询所有未删除的数据
        Optional<${class_name}> ${object_name} = ${object_name}Repository.findByIdAndDelflag(${object_name}Dto.getId(), DeleteFlag.UN_DELETED);
        ...
        //数据更新
        ${object_name}DataService.update(entity);
        //创建事件构造器对象
        ${class_name}EventBuilder builder = new ${class_name}EventBuilder();
        //事件发送者发送更新事件
    ...
    public void delete${class_name}ById(Long id){
       //根据唯一编码从数据库查询所有未删除的数据
        Optional<${class_name}> ${object_name} = ${object_name}Repository.findByIdAndDelflag(id, DeleteFlag.UN_DELETED);
        if(!${object_name}.isPresent()){
            //抛出资源没有找到的异常
            throw new ResourceNotFoundException(ExceptionMessageFormat.idNotFoundMessage(id));
        }
        ...
        ${class_name} entity = ${object_name}.get();
        // 设置删除标志位true
        entity.setDelflag(DeleteFlag.IS_DELETED);
        //数据删除
        ${object_name}DataService.delete(entity);
        //创建事件构造器对象
        ${class_name}EventBuilder builder = new ${class_name}EventBuilder();
        //事件发送者发送更新事件
        eventSender.sendDefaultMessage(builder.buildUpdateEvent(entity));
