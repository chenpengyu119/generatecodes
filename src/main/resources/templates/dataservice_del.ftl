...
public class ${class_name}DataService {
     ...
     */
    @Cacheable(key = "#id")
    public Optional<${class_name}Dto> findByIdAndDelflag(Long id, Boolean delflag){
        Optional<${class_name}> ${object_name} = ${object_name}Repository.findByIdAndDelflag(id, delflag);
    ...
    /**
     * 删除${key_annotation}信息
     * @param ${object_name} ${key_annotation}
     * @return  ${key_annotation}对象
     */
    @CacheEvict(key = "#result.id")
    public ${class_name}Dto delete(${class_name} ${object_name}) {
        ${object_name}Repository.save(${object_name});
        return BeanUtils.copyProperties(${object_name}, ${class_name}Dto.class);
    }
